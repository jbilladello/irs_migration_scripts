# This script works with CSV files for County-level IRS migration data from the years 2013 - 2014 forward.
# It loops through the county inflow and county outflow files, formats their rows into a list,
# creates summary tables for 'totals' records, and exports the lists to a SQLite database.

# Users will be prompted to enter the full path to the folder in which the data files are stored,
# confirm that the table names are accurate for the year being processed, and name the SQLite database.
...
for item in os.listdir(path_to_county_data):
    if item [0:8] == 'countyin':
        infilepath=os.path.join(path_to_county_data,item)
        in_years = item[-8:-4]
        ...
        outfilepath=os.path.join(path_to_county_data,item)
        out_years = item[-8:-4]
...
db_name = os.path.join(path_to_county_data,input()) + '.sqlite'
...
with open(infilepath) as counties:   
    newlist_in = []
    newlist_totals = []
...
#--outflow--#
with open(outfilepath) as counties:
    newlist_out = []
    newlist_totals_out = []
    headers = counties.readline().strip().split(',') #skips first line
    for line in counties:
